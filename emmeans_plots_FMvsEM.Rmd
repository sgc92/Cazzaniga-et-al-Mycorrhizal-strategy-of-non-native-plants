...
```{r}
mod_status_AMNMvsEM <- readRDS("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/mod_status_AMNMvsEM.rds")
summary(mod_status_AMNMvsEM)
Anova(mod_status_AMNMvsEM)
performance::model_performance(mod_status_AMNMvsEM)
```

```{r}
#extract model means
#means12 <- emmeans (mod_status_AMNMvsEM, ~ status , component = "response", cov.reduce = mean, data = dat)
means12 <- readRDS("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/means_status_AMNMvsEM.rds")
#calculate significant differences
cld_sign12 <- as.data.frame(cld(means12, Letters = letters, adjust = "BH", alpha = 0.05, decreasing = TRUE))
cld_sign12

#plot emmeans
plot_mean_status_AMNMvsEM <- ggplot(data = cld_sign12, aes (x = status, y = emmean)) +
...
plot_mean_status_AMNMvsEM
ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/mod_status_means_AMNMvsEM.svg", plot=plot_mean_status_AMNMvsEM, width=10, height=8, scale = 0.5)

s12<- summary(mod_status_AMNMvsEM)
...
plot_env_AMNMvsEM_status <- ggplot(df12, mapping =  aes(x = Estimate, y = env)) +
  ...
  theme_minimal()
plot_env_AMNMvsEM_status
ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/mod_status_AMNMvsEM_env.svg", plot=plot_env_AMNMvsEM_status, width=10, height=7, scale = 0.5)
```

Make dataset for comparison
```{r}
#extract from model with status = global
status_diff_AMNMvsEM <- cld_sign12 %>%
...
## with env variables
```{r}
mod_biome_AMNMvsEM <- readRDS("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/mod_biome_AMNMvsEM.rds")
summary(mod_biome_AMNMvsEM) #AIC 1108308.3
Anova(mod_biome_AMNMvsEM)
performance::model_performance(mod_biome_AMNMvsEM)
```
...
mod_biome_AMNMvsEM <- readRDS("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/mod_biome_AMNMvsEM.rds")
#extract model means
#means32 <- emmeans (mod_biome_AMNMvsEM, ~ status | biome , component = "response", cov.reduce = mean, data = dat.sel)
means32 <- readRDS("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/means_biome_AMNMvsEM.rds")
#calculate significant differences
cld_sign32 <- as.data.frame(cld(means32, Letters = letters, adjust = "BH", alpha = 0.05, decreasing = TRUE))
cld_sign32

s32<- summary(mod_biome_AMNMvsEM)
...
plot_env_AMNMvsEM_biome <- ggplot(df32, aes(x = Estimate, y = env)) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey", size = 0.8) +  # Dashed line at 0
  geom_point(aes(color = env), size = 2) +  # Add points with color (for better distinction)
  geom_errorbarh(aes(xmin = Estimate - `Std. Error`, xmax = Estimate + `Std. Error`), 
  ...
  theme(axis.text.y = element_text(size = 10),  # Adjust the y-axis text size
        ...
        panel.grid.minor = element_blank()) + # Remove minor grid linesggplot(df12, mapping = aes(x = Estimate, y = env)) 
  theme_minimal()
plot_env_AMNMvsEM_biome

ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/mod_biome_AMNMvsEM_status_ selectedbiomes_env.svg", plot=plot_env_AMNMvsEM_biome, width=10, height=7, scale = 0.5)
```
...
df_diff_AMNMvsEM_biome <- cld_sign32 %>% 
...
#merge global + biome
df_diff_AMNMvsEM_biome_status <- full_join(status_diff_AMNMvsEM, df_diff_AMNMvsEM_biome, by = NULL)
df_diff_AMNMvsEM_biome_status <- df_diff_AMNMvsEM_biome_status %>% 
...
background_bands <- data.frame (
  biome = levels (df_diff_AMNMvsEM_biome_status$biome),
  y_min = seq (0.5, length(levels(df_diff_AMNMvsEM_biome_status$biome)) - 0.5, by = 1),
  y_max = seq(1.5, length(levels(df_diff_AMNMvsEM_biome_status$biome)) + 0.5, by = 1),
  fill_color = rep (c("white", "lightgray"), length.out = length(df_diff_AMNMvsEM_biome_status$biome)))
...
df_diff_AMNMvsEM_biome_status

#order HD no biome simp - biome + dist
plot_AMNMvsEM2 <- ggplot(data = df_diff_AMNMvsEM_biome_status, 
...
plot_AMNMvsEM2
#save
ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/mod_biome_difference_AMNMvsEM_status_biomes.png", plot=plot_AMNMvsEM2, width=15, height=7, scale = 0.5,  dpi = 300)

ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/mod_biome_difference_AMNMvsEM_status_biomes.svg",plot=plot_AMNMvsEM2, width=15, height=7, scale = 0.5)
```
