...
```{r}
mod_global_NMvsEM <- readRDS("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/all_rar/mod_global_NMvsEM_rarall.rds")
summary(mod_global_NMvsEM)
Anova(mod_global_NMvsEM)
performance::model_performance(mod_global_NMvsEM)
```

```{r}
#extract model means
#means12 <- emmeans (mod_global_NMvsEM, ~ status , component = "response", cov.reduce = mean, data = dat)
means12 <- readRDS ("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/all_rar/means_global_NMvsEM_rarall.rds")
#calculate significant differences
cld_sign12 <- as.data.frame(cld(means12, Letters = letters, adjust = "BH", alpha = 0.05, decreasing = TRUE))
cld_sign12

#plot emmeans
plot_mean_status_NMvsEM <- ggplot(data = cld_sign12, aes (x = status, y = emmean)) +
...
plot_mean_status_NMvsEM
ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/all_rar/mod_global_means_NMvsEM_rar.svg", plot=plot_mean_status_NMvsEM, width=10, height=8, scale = 0.5)

s12<- summary(mod_global_NMvsEM)
...
plot_env_NMvsEM_status <- ggplot(df12, mapping =  aes(x = Estimate, y = env)) +
  ...
  theme_minimal()
plot_env_NMvsEM_status
ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/all_rar/mod_global_NMvsEM_env_rar.svg", plot=plot_env_NMvsEM_status, width=10, height=7, scale = 0.5)
```

Make dataset for comparison
```{r}
#extract from model with status = global
status_diff_NMvsEM <- cld_sign12 %>%
  ...
  mutate(prev_status = case_when(
    ...
    mean_diff < 0 ~ "more EM"                                   # Negative mean_diff
  )) %>% 
...
## with env variables
```{r}
mod_biome_NMvsEM <- readRDS("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/all_rar/mod_biome_NMvsEM_rarall.rds")
summary(mod_biome_NMvsEM) #AIC 1108308.3
Anova(mod_biome_NMvsEM)
performance::model_performance(mod_biome_NMvsEM)
```


```{r}
#extract model means
#means32 <- emmeans (mod_biome_NMvsEM, ~ status | biome , component = "response", cov.reduce = mean, data = dat.sel)
means32 <- readRDS ("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/all_rar/means_biome_NMvsEM_rarall.rds")
#calculate significant differences
cld_sign32 <- as.data.frame(cld(means32, Letters = letters, adjust = "BH", alpha = 0.05, decreasing = TRUE))
cld_sign32

s32<- summary(mod_biome_NMvsEM)
...
plot_env_NMvsEM_biome <- ggplot(df32, aes(x = Estimate, y = env)) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey", size = 0.8) +  # Dashed line at 0
  geom_point(aes(color = env), size = 2) +  # Add points with color (for better distinction)
  geom_errorbarh(aes(xmin = Estimate - `Std. Error`, xmax = Estimate + `Std. Error`), 
  ...
  theme(axis.text.y = element_text(size = 10),  # Adjust the y-axis text size
        ...
        panel.grid.minor = element_blank()) + # Remove minor grid linesggplot(df12, mapping = aes(x = Estimate, y = env)) 
  theme_minimal()
plot_env_NMvsEM_biome

ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/all_rar/mod_biome_NMvsEM_status_ selectedbiomes_env_rar.svg", plot=plot_env_NMvsEM_biome, width=10, height=7, scale = 0.5)
```
...
df_diff_NMvsEM_biome <- cld_sign32 %>% 
...
#merge global + biome
df_diff_NMvsEM_biome_status <- full_join(status_diff_NMvsEM, df_diff_NMvsEM_biome, by = NULL)
df_diff_NMvsEM_biome_status <- df_diff_NMvsEM_biome_status %>% 
...
background_bands <- data.frame (
  biome = levels (df_diff_NMvsEM_biome_status$biome),
  y_min = seq (0.5, length(levels(df_diff_NMvsEM_biome_status$biome)) - 0.5, by = 1),
  y_max = seq(1.5, length(levels(df_diff_NMvsEM_biome_status$biome)) + 0.5, by = 1),
  fill_color = rep (c("white", "lightgray"), length.out = length(df_diff_NMvsEM_biome_status$biome)))
...
df_diff_NMvsEM_biome_status

#order HD no biome simp - biome + dist
plot_NMvsEM2 <- ggplot(data = df_diff_NMvsEM_biome_status, 
...
plot_NMvsEM2
#save
ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/all_rar/mod_biome_difference_NMvsEM_status_biomes_rar.png", plot=plot_NMvsEM2, width=15, height=7, scale = 0.5,  dpi = 300)

ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/all_rar/mod_biome_difference_NMvsEM_status_biomes_rar.svg",plot=plot_NMvsEM2, width=15, height=7, scale = 0.5)
```
