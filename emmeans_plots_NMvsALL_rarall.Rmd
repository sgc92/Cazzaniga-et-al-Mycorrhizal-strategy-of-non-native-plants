...
```{r}
mod_status_NMvsALL <- readRDS("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/all_rar/mod_global_NMvsALL_rarall.rds")
summary(mod_status_NMvsALL)
Anova(mod_status_NMvsALL)
performance::model_performance(mod_status_NMvsALL)
```

```{r}
#extract model means
#means12 <- emmeans (mod_status_NMvsALL, ~ status , component = "response", cov.reduce = mean, data = dat)
means12 <- readRDS ("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/all_rar/means_global_NMvsALL_rarall.rds")
#calculate significant differences
cld_sign12 <- as.data.frame(cld(means12, Letters = letters, adjust = "BH", alpha = 0.05, decreasing = TRUE))
cld_sign12

#plot emmeans
plot_mean_status_NM <- ggplot(data = cld_sign12, aes (x = status, y = emmean)) +
...
plot_mean_status_NM
ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/all_rar/mod_status_means_NMvsALL_rar.svg", plot=plot_mean_status_NM, width=10, height=8, scale = 0.5)

s12<- summary(mod_status_NMvsALL)
...
plot_env_NM_status <- ggplot(df12, mapping =  aes(x = Estimate, y = env)) +
  ...
  theme_minimal()
plot_env_NM_status
ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/all_rar/mod_status_NMvsALL_env_rar.svg", plot=plot_env_NM_status, width=10, height=7, scale = 0.5)
```
...
## with env variables
```{r}
mod_biome_NMvsALL <- readRDS("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/all_rar/mod_biome_NMvsALL_rarall.rds")
summary(mod_biome_NMvsALL) #AIC 1108308.3
Anova(mod_biome_NMvsALL)
performance::model_performance(mod_biome_NMvsALL)
```


```{r}
means32 <- readRDS("C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Models/all_rar/means_biome_NMvsALL_rarall.rds")
#extract model means
#means32 <- emmeans (mod_biome_NMvsALL, ~ status | biome , component = "response", cov.reduce = mean, data = dat.sel)
#calculate significant differences
cld_sign32 <- as.data.frame(cld(means32, Letters = letters, adjust = "BH", alpha = 0.05, decreasing = TRUE))
cld_sign32

s32<- summary(mod_biome_NMvsALL)
...
plot_env_NM_biome <- ggplot(df32, aes(x = Estimate, y = env)) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey", size = 0.8) +  # Dashed line at 0
  geom_point(aes(color = env), size = 2) +  # Add points with color (for better distinction)
  geom_errorbarh(aes(xmin = Estimate - `Std. Error`, xmax = Estimate + `Std. Error`), 
...
ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/all_rar/mod_biome_NMvsAll_status_ selectedbiomes_env_rar.svg", plot=plot_env_NM_biome, width=10, height=7, scale = 0.5)
```
...
df_diff_NMall_biome <- cld_sign32 %>% 
...
#merge global + biome
df_diff_NMall_biome_status <- full_join(status_diff_NM, df_diff_NMall_biome, by = NULL)
df_diff_NMall_biome_status <- df_diff_NMall_biome_status %>% 
...
background_bands <- data.frame (
  biome = levels (df_diff_NMall_biome_status$biome),
  y_min = seq (0.5, length(levels(df_diff_NMall_biome_status$biome)) - 0.5, by = 1),
  y_max = seq(1.5, length(levels(df_diff_NMall_biome_status$biome)) + 0.5, by = 1),
  fill_color = rep (c("white", "lightgray"), length.out = length(df_diff_NMall_biome_status$biome)))
...
df_diff_NMall_biome_status

#order HD no biome simp - biome + dist
plot_NMall2 <- ggplot(data = df_diff_NMall_biome_status, 
...
plot_NMall2
#save
ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/all_rar/mod_biome_difference_NMvsALL_status_biomes_rar.png", plot=plot_NMall2, width=15, height=7, scale = 0.5,  dpi = 300)

ggsave(file="C:/Users/sarag/OneDrive - ETH Zurich/1_Invasion/Inv_Data/sPlot4_Sara/Figures/all_rar/mod_biome_difference_NMvsALL_status_biomes_rar.svg",plot=plot_NMall2, width=15, height=7, scale = 0.5)
```
